...
export class PedidosComponent implements OnInit {
  ...
  save() {
    ...
    if (this.verificarCamposObrigatorios()) {
      if (this.isNew) {
        this.pedidoService.create(this.pedido, this.pedido.cliente.id).subscribe(response => {
          this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Pedido criado com sucesso!', life: 3000 });
          this.fecharDialog();
        })
      } else {
        this.pedidoService.update(this.pedido, this.pedido.id).subscribe(response => {
